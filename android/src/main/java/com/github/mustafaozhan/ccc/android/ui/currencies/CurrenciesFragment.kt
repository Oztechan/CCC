...
class CurrenciesFragment : BaseVBFragment<FragmentCurrenciesBinding>() {
    ...
    private fun observeStates() = currenciesViewModel.state
        ...
        .onEach { state ->
            currenciesAdapter.submitList(state.currencyList)

            binding.loadingView.showLoading(state.loading)

            with(binding.layoutCurrenciesToolbar) {
                searchView.visibleIf(!state.selectionVisibility)
                txtCurrenciesToolbar.visibleIf(!state.selectionVisibility)
                btnSelectAll.visibleIf(state.selectionVisibility)
                btnDeSelectAll.visibleIf(state.selectionVisibility)
                backButton.visibleIf(!currenciesViewModel.isFirstRun() || state.selectionVisibility)

                backButton.setBackgroundResource(
                    if (state.selectionVisibility) R.drawable.ic_close else R.drawable.ic_back
                )
                toolbarFragmentCurrencies.setBackgroundColor(
                    ContextCompat.getColor(
                        requireContext(),
                        if (state.selectionVisibility) R.color.color_background_weak else R.color.color_background_strong
                    )
                )
            }
    ...
    private fun observeEffect() = currenciesViewModel.effect
        .flowWithLifecycle(lifecycle)
        .onEach { effect ->
            kermit.d { "CurrenciesFragment observeEffect ${effect::class.simpleName}" }
            when (effect) {
